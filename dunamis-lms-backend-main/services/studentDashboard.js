...
// Everything the student Overview and Performance pages show (and the chatbot's
// personal answers), in one round trip. Takes the Student _id.
async function buildStudentDashboard(studentId) {
  ...
  const [upcomingSlots, assignments, assessments, attendance, certificateCount] = await Promise.all([
    Slot.find({
      ...
      .limit(6)
      .lean(),
    Assignment.find({ "students.studentId": studentId })
    ...
    Assessment.find({ studentId })
      ...
      .sort({ dueDate: 1 })
      .lean(),
    AttendanceHomework.find({ studentId })
  ...
  const mine = (assignment) =>
    (assignment.students || []).find((s) => String(s.studentId) === String(studentId));
...
module.exports = { buildStudentDashboard };
