...
import { HiChevronLeft, HiChevronRight, HiClock, HiUser } from "react-icons/hi";
...
export default function DemoSlotCalendar({ slots, selectedId, onSelect }) {
  const byDay = useMemo(() => {
    const map = new Map();
    slots.forEach((slot) => {
  ...
  useEffect(() => {
    ...
    const [year, month, day] = availableDays[0].split("-").map(Number);
  ...
  const bounds = useMemo(() => {
    ...
    return { first: toMonth(availableDays[0]), last: toMonth(availableDays[availableDays.length - 1]) };
  }, [availableDays]);
      ...
      <div className="rounded-2xl border border-stone-200 p-4">
        <div className="flex items-center justify-between">
          <button
            ...
            className="rounded-full p-1.5 text-slate-600 transition hover:bg-stone-100 disabled:opacity-30"
          >
              ...
              <button
                ...
                className={`relative flex h-10 flex-col items-center justify-center rounded-xl text-sm transition ${
                  isSelected
                    ...
                    : hasSlots
                    ? "font-semibold text-slate-800 hover:bg-orange-50"
                    : "text-slate-300"
                }`}
              >
                {cell.dayOfMonth}
